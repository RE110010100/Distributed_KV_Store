...
func (s *Server) GetHandler(w http.ResponseWriter, r *http.Request) {
	...
	shard_index := s.shards.Index(key)
	value, err := s.db.GetKey(key)

	if shard_index != s.shards.CurIdx {
		s.redirect(shard_index, w, r)
		return
	}

	fmt.Fprintf(w, "Shard = %d, current shard = %d, addr = %q, Value = %q, error = %v", shard_index, s.shards.CurIdx, s.shards.Addrs[shard_index], value, err)
}
