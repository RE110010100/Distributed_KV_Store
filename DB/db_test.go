...
func TestGetSet(t *testing.T) {
	...
	if err := db.SetKey("hello", []byte("world")); err != nil {
		t.Fatalf("could not write key : %v", err)
	}

	value, err := db.GetKey("hello")

	if err != nil {
		t.Fatalf("could not get the value for the key : %v", err)
	}

	if !bytes.Equal(value, []byte("world")) {
		t.Errorf("unexpected value for the key 'hello' : got %v wanted 'world'", value)
	}
	...
	if !bytes.Equal(k, []byte("hello")) || !bytes.Equal(v, []byte("world")) {
		t.Errorf(`GetNextKeyForReplication(): got %q, %q; want %q, %q`, k, v, "hello", "world")
	}
